...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit('`[HEROKU]:'
                         '\nSedang MengUpdate ⚡SkyXReez Projects⚡, Mohon Menunggu 5-7 Menit`'
                         )
        ups_rem.fetch(ac_br)
        ...
        if build.status == "failed":
            ...
            await event.edit("`Rest-ProjectBot Berhasil Di Deploy!\n" "Restarting, Mohon Tunggu Sebentar.....`")
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit('**⚡Rest-Project⚡** `Berhasil Di Update!`')
    await asyncio.sleep(1)
    await event.edit('**⚡Rest-Project⚡** `Di Restart....`')
    await asyncio.sleep(1)
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#BOT \n"
            "**⚡Rest-Project⚡ Telah Di Perbarui.**")
...
async def upstream(event):
    ...
    if changelog == '' and force_update is False:
        await event.edit(
            f'\n⚡Rest-Project⚡ Sudah Versi Terbaru\n')
        await asyncio.sleep(15)
        await event.delete()
        return repo.__del__()

    if conf is None and force_update is False:
        changelog_str = f'**Pembaruan Untuk ⚡Rest-Project⚡ :\n\n💻 Pembaruan Data :**\n`{changelog}`'
    ...
    if force_update:
        ...
        await event.edit('` Proses Update Kontol Restu, Loading....1%`')
        await event.edit('` Proses Update Kontol Restu, Loading....20%`')
        await event.edit('` Proses Update Kontol Restu, Loading....35%`')
        await event.edit('` Proses Update Kontol Restu, Loading....77%`')
        await event.edit('` Proses Update Kontol Restu, Updating...90%`')
        await event.edit('` Proses Update Kontol Restu, Mohon Tunggu Sebentar....100%`')
    if conf == "now":
...
CMD_HELP.update({
    ...
    "\n\n𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.update now`"
    "\n• : Memperbarui Rest-Project."
    "\n\n𝘾𝙤𝙢𝙢𝙖𝙣𝙙: `.update deploy`"
    "\n• : Memperbarui Rest-Project Dengan Cara Men-Deploy Ulang."
})
